...
//modulo espacios
	//iniciar sesion
int registro ( FILE*ar, FILE*pass, int gg)
{
	char nom1 [10];
	char nom2 [10];
	...
	gets (nom1);
	fread (&us.usuario, sizeof(user), 1, ar);
		while (!feof(ar))
			{
				if (strcmp (nom1,us.usuario)==0)
				{
					_flushall();
					rewind(ar);
					printf("\nIngrese su contraseña");
					gets (nom2); 
					fread (&us.pass, sizeof(user), 1, ar);
					while (!feof(ar))
					{
						if (strcmp (nom1,us.pass)==0)
						{
							gg=1; 
							return (gg);
						}
...
int tratamiento (FILE* ficha, FILE* archi, aux turnos){
	...
	while (!feof(archi))
	{
		...
		scanf ("%d", &turnos.dni);
		if (turnos.dni == regi.dni)
		{
			turnos.fech.dia = regi.fech.dia;
			turnos.fech.mes = regi.fech.mes;
			turnos.fech.year = regi.fech.year;
			turnos.idprof = regi.idprof;
			 strcpy(turnos.detaten, regi.detaten );
		}
		else
		fread (&regi, sizeof(regi),1, archi);	
	}	
	...
	printf ("\n Escriba su nombre: ");
		gets (auxnom);
		fprintf(ficha, " \n Dia: %d / Mes: %d / Anio: %d", turnos.fech.dia, turnos.fech.mes, turnos.fech.year );
	_flushall();
	printf("\n Evolucion del paciente (por favor no exceder los 380 caracteres) \n ");	
		gets (evolucion);
		fprintf (ficha, "prescripción: %s \n\n evolucion: %s", turnos.detaten, evolucion );
	fclose(ficha);
